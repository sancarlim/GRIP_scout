...
def preprocess_data(pra_data, pra_rescale_xy):
	...
	#pos,heading,vel,recording_id,frame,id, l,w, class, mask ->POS HEAD MASK
	feature_id = [0, 1, 2, 11]  #[3,4,9,10]
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
	...
	for iteration, (ori_data, A, _) in enumerate(pra_data_loader):
		...
		# ori_data: (N, C, T, V)
		# C = 11: [frame_id, object_id, object_type, position_x, position_y, position_z, object_length, pbject_width, pbject_height, heading] + [mask]
		#pos,heading,vel,recording_id,frame,id, l,w, class, mask C=12
		data, no_norm_loc_data, object_type = preprocess_data(ori_data, rescale_xy)
		for now_history_frames in range(1, history_frames+1):
			...
			predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=output_loc_GT.shape[-2]) # (N, C, T, V)=(N, 2, 6, 120)
...
def val_model(pra_model, pra_data_loader):
	...
	for iteration, (ori_data, A, _) in enumerate(pra_data_loader):
		...
		for now_history_frames in range(history_frames, history_frames+1):
			...
			A = A.float().to(dev)
			predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=future_frames) # (N, C, T, V)=(N, 2, 6, 120)
			########################################################
...
def run_test(pra_model, pra_data_path):
	loader_test = data_loader(pra_data_path, pra_batch_size=32, pra_shuffle=False, pra_drop_last=False, train_val_test='test')
	display_result(
...
if __name__ == '__main__':
	...
	model = Model(in_channels=4, graph_args=graph_args, edge_importance_weighting=False)
	model.to(dev)

	#train and evaluate model
	#run_trainval(model, pra_traindata_path='/media/14TBDISK/sandra/inD_processed/inD_GRIP_train_step8.pkl' , pra_testdata_path='/media/14TBDISK/sandra/inD_processed/inD_inD_GRIP_train_step8.pkl')
	
	pretrained_model_path = './trained_models/model_epoch_0029.pt'
	model = my_load_model(model, pretrained_model_path)
	run_test(model, '/media/14TBDISK/sandra/inD_processed/inD_GRIP_test.pkl')
